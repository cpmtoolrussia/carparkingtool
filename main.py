...
RANK_URLS = [
    "https://us-central1-cp-multiplayer.cloudfunctions.net/SetUserRating1",
    "https://us-central1-cp-multiplayer.cloudfunctions.net/SetUserRating2",
    "https://us-central1-cp-multiplayer.cloudfunctions.net/SetUserRating5",
...
class CPMNuker:
    ...
    async def _post(self, url, payload, headers):
        try:
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30), connector=aiohttp.TCPConnector(ssl=False)) as s:
    ...
    async def load(self, uid, force=False):
        ...
        if not ok: 
            print(f"DEBUG [{uid}]: Auth failed: {msg}")
            return False
        try:
            print(f"DEBUG [{uid}]: Requesting data...")
            r = await self._post(LOAD_URL, {"data": None}, {**GAME_HEADERS, "Authorization": f"Bearer {auth}"})
            if not r: return False
            raw_data = r.get("result") or r.get("data") or r.get("raw") if isinstance(r, dict) else r
    ...
    def _ok(self, v):
        if v in (1,True,"1"): return True
        if isinstance(v, dict) and v.get("result") == "1": return True
        return False
    ...
    async def set_rank(self, uid):
        ...
        rd = {"RatingData":{"time":1e22,"cars":1e16,"car_fix":1e13,"car_collided":1e12,
            ...
            "burnt_tire":1e10,"passanger_distance":1e10,"online_time":1e22}}
        for url in RANK_URLS:
            try:
                r = await self._post(url,rd,{**GAME_HEADERS,"Authorization":f"Bearer {auth}"})
                if r and self._ok(r): return {"ok":True}
            except: continue
        return {"ok":False,"message":"All rank servers are down."}
    ...
    async def _fetch_record(self, auth, fuid, password=None, email=None):
        try:
            ...
            dec = decrypt_player_record(raw, fuid, password, email)
    ...
    async def _signup(self, email, password):
        ...
        try:
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as s:
                async with s.post(url, json={"email": email, "password": password, "returnSecureToken": True}) as resp:
                    r = await resp.json()
                    if r.get("idToken"):
    ...
    async def multi_clone_accounts(self, donor_email, donor_pass, domain, count, new_password):
        donor_res = await self.login(donor_email, donor_pass)
        if not donor_res.get("ok"): return {"ok": False, "message": "DONOR_LOGIN_FAIL"}
        donor_data = await self._fetch_record(donor_res["auth"], donor_res["firebase_uid"], donor_pass, donor_email)
        if not donor_data: return {"ok": False, "message": "DONOR_DATA_FAIL"}
        created = []; failed = []
        import random, string
        fields = ["money", "coin", "allData", "boughtFsos", "boughtPoliceLights", "boughtPoliceSirens", "fcar", "animations", "wheels"]
        for i in range(count):
            email = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8)) + domain
            reg = await self._signup(email, new_password)
            if reg.get("ok"):
                clone = {f: deepcopy(donor_data[f]) for f in fields if f in donor_data}
                clone["Name"] = f"AlexGG_{random.randint(100,999)}"
                clone["localID"] = "GG" + ''.join(random.choices(string.ascii_uppercase, k=6))
                await self._send(reg["auth"], clone, reg["firebase_uid"])
                created.append({"email": email, "pass": new_password})
            else: failed.append({"email": email, "pass": new_password})
...
class SMultiClone(StatesGroup):
    donor_email = State()      # Email донора
    donor_pass = State()       # Пароль донора
    domain = State()           # Домен почты (@gmail.com и т.д.)
    count = State()            # Количество аккаунтов
    password = State()         # Пароль для новых аккаунтов
...
# ═══════════════════════════════════════════
# ═══════════════════════════════════════════
#  🧬 CLONE & MASS CLONE HANDLERS
# ═══════════════════════════════════════════

@rt.callback_query(F.data == "f_clone")
async def cb_clone_start(cb: CallbackQuery, state: FSMContext):
    if not nuker.get_token(cb.from_user.id): await cb.answer("⛔ Войдите!", show_alert=True); return
    await state.set_state(SClone.target_email)
    await cb.message.edit_text(f"{B}\n🧬 CLONE ACCOUNT\n{B}\n\n  ▸ Введите EMAIL цели:", reply_markup=K.cancel())

@rt.message(SClone.target_email)
async def p_clone_email(msg: Message, state: FSMContext):
    await state.update_data(target_email=msg.text.strip()); await state.set_state(SClone.target_password)
    await msg.answer("🔑 Введите ПАРОЛЬ цели:", reply_markup=K.cancel())

@rt.message(SClone.target_password)
async def p_clone_pass(msg: Message, state: FSMContext):
    data = await state.get_data(); status = await msg.answer("⏳ Клонирование..."); await msg.delete()
    try:
        donor_uid = msg.from_user.id; await nuker.load(donor_uid, force=True); donor_data = nuker.get_record(donor_uid)
        login_res = await nuker.login(data["target_email"], msg.text.strip())
        if not login_res.get("ok"): await status.edit_text("❌ Ошибка входа."); return
        target_raw = await nuker._fetch_record(login_res["auth"], login_res["firebase_uid"], msg.text.strip(), data["target_email"])
        fields = ["money", "coin", "allData", "boughtFsos", "boughtPoliceLights", "boughtPoliceSirens", "fcar", "animations", "wheels"]
        final = deepcopy(target_raw) if target_raw else {}
        for f in fields: 
            if f in donor_data: final[f] = deepcopy(donor_data[f])
        if target_raw:
            for f in ["localID", "Name", "personEquipmentsMale", "personEquipmentsFemale", "FriendsID"]:
                if f in target_raw: final[f] = target_raw[f]
        success, _ = await nuker._send(login_res["auth"], final, login_res["firebase_uid"])
        if success:
            nuker.save_token(donor_uid, login_res["auth"], data["target_email"], msg.text.strip(), login_res["refresh_token"], login_res["firebase_uid"])
            await status.edit_text("✅ Клонирование успешно!", reply_markup=K.home(donor_uid))
        else: await status.edit_text("❌ Ошибка сохранения.")
...
async def cb_mass_clone_start(cb: CallbackQuery, state: FSMContext):
    if not nuker.get_token(cb.from_user.id): await cb.answer("⛔ Войдите!", show_alert=True); return
    await state.set_state(SMassClone.donor_email)
    await cb.message.edit_text(f"{B}\n🔥 MASS CLONE\n{B}\n\n  ▸ Введите EMAIL донора:", reply_markup=K.cancel())

@rt.message(SMassClone.donor_email)
async def p_mass_donor_email(msg: Message, state: FSMContext):
    await state.update_data(donor_email=msg.text.strip()); await state.set_state(SMassClone.donor_pass)
    await msg.answer("🔑 Введите ПАРОЛЬ донора:", reply_markup=K.cancel())

@rt.message(SMassClone.donor_pass)
async def p_mass_donor_pass(msg: Message, state: FSMContext):
    await state.update_data(donor_pass=msg.text.strip()); await state.set_state(SMassClone.domain)
    await msg.delete(); await msg.answer("🌐 Введите ДОМЕН (например @gmail.com):", reply_markup=K.cancel())

@rt.message(SMassClone.domain)
async def p_mass_domain(msg: Message, state: FSMContext):
    await state.update_data(domain=msg.text.strip()); await state.set_state(SMassClone.count)
    await msg.answer("🔢 Введите КОЛИЧЕСТВО (max 50):", reply_markup=K.cancel())

@rt.message(SMassClone.count)
async def p_mass_count(msg: Message, state: FSMContext):
    try:
        c = int(msg.text.strip()); assert 1 <= c <= 50
        await state.update_data(count=c); await state.set_state(SMassClone.password)
        await msg.answer("🔑 Введите ПАРОЛЬ для новых аккаунтов:", reply_markup=K.cancel())
    except: await msg.answer("❌ Введите число 1-50.")

@rt.message(SMassClone.password)
async def p_mass_password(msg: Message, state: FSMContext):
    data = await state.get_data(); count = data.get("count"); domain = data.get("domain")
    donor_em = data.get("donor_email"); donor_pw = data.get("donor_pass"); new_pw = msg.text.strip()
    await msg.delete()
    
    status_msg = await msg.answer("◐ Bulk Clone Progress\n\n📝 Obtained: 0/0 (0%)\n░░░░░░░░░░░░\n\n🚗 Cloning: 0/0 (0%)\n░░░░░░░░░░░░\n\n📦 Pool: 0")
    ...
    try:
        donor_res = await nuker.login(donor_em, donor_pw)
        if not donor_res.get("ok"):
            await status_msg.edit_text("❌ Donor Login Failed."); return
            
        donor_data = await nuker._fetch_record(donor_res["auth"], donor_res["firebase_uid"], donor_pw, donor_em)
        if not donor_data:
            await status_msg.edit_text("❌ Could not fetch donor data."); return

        # Progress 1: Obtained
        for i in range(1, 11):
            p = i * 10
            await status_msg.edit_text(f"◐ Bulk Clone Progress\n\n📝 Obtained: {i}/10 ({p}%)\n{get_bar(p)}\n\n🚗 Cloning: 0/{count} (0%)\n░░░░░░░░░░░░\n\n📦 Pool: 0")
            await asyncio.sleep(0.1)

        fields = ["money", "coin", "allData", "boughtFsos", "boughtPoliceLights", "boughtPoliceSirens", "fcar", "animations", "wheels"]
        created = []; pool_count = 0
        import random, string

        for i in range(1, count + 1):
            email = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8)) + domain
            reg = await nuker._signup(email, new_pw)
            if reg.get("ok"):
                clone = {f: deepcopy(donor_data[f]) for f in fields if f in donor_data}
                clone["Name"] = f"AlexGG_{random.randint(100,999)}"
                clone["localID"] = "GG" + ''.join(random.choices(string.ascii_uppercase, k=6))
                await nuker._send(reg["auth"], clone, reg["firebase_uid"])
                created.append(f"{email}:{new_pw}")
                pool_count += random.randint(100, 500) # Simulating pool size increment
        ...
        if created:
            file = BufferedInputFile("\n".join(created).encode(), filename="accounts.txt")
            await msg.answer_document(file, caption=f"✅ Bulk Clone Finished\n📦 Total Pool: {pool_count}")
        
        await status_msg.edit_text("✅ All accounts created successfully!", reply_markup=K.home(msg.from_user.id))
    except Exception as e:
