...
async def cb_mass_clone_start(cb: CallbackQuery, state: FSMContext):
    if not nuker.get_token(cb.from_user.id): await cb.answer("✗ Sign in first!", show_alert=True); return
    await state.set_state(SMultiClone.donor_email)
    await cb.message.edit_text(
        f"{B}\n  🔥  𝗠𝗔𝗦𝗦 𝗖𝗟𝗢𝗡𝗘\n{B}\n\n"
        "  ▸ Enter the DONOR's email:",
        reply_markup=K.cancel()
    )
    await cb.answer()

@rt.message(SMultiClone.donor_email)
async def p_mass_donor_email(msg: Message, state: FSMContext):
    email = msg.text.strip()
    if "@" not in email or "." not in email:
        await msg.answer("  ✗ Invalid email.", reply_markup=K.cancel())
        return
    await state.update_data(donor_email=email)
    await state.set_state(SMultiClone.donor_pass)
    await msg.answer(
        f"{B}\n  🔥  𝗠𝗔𝗦𝗦 𝗖𝗟𝗢𝗡𝗘\n{B}\n\n"
        f"  ▸ Enter the DONOR's password:",
        reply_markup=K.cancel()
    )

@rt.message(SMultiClone.donor_pass)
async def p_mass_donor_pass(msg: Message, state: FSMContext):
    password = msg.text.strip()
    try: await msg.delete()
    except: pass
    await state.update_data(donor_pass=password)
    await state.set_state(SMultiClone.domain)
    await msg.answer(
        f"{B}\n  🔥  𝗠𝗔𝗦𝗦 𝗖𝗟𝗢𝗡𝗘\n{B}\n\n"
        "  ▸ Enter the email domain for new accounts:\n"
        "  (e.g., @gmail.com, @outlook.com, @yandex.ru)",
        reply_markup=K.cancel()
    )

@rt.message(SMultiClone.domain)
async def p_mass_domain(msg: Message, state: FSMContext):
    domain = msg.text.strip()
    if not domain.startswith("@"):
        domain = "@" + domain
    if len(domain) < 3:
        await msg.answer("  ✗ Invalid domain.", reply_markup=K.cancel())
        return
    await state.update_data(domain=domain)
    await state.set_state(SMultiClone.count)
    await msg.answer(
        f"{B}\n  🔥  𝗠𝗔𝗦𝗦 𝗖𝗟𝗢𝗡𝗘\n{B}\n\n"
...
@rt.message(SMultiClone.count)
async def p_mass_count(msg: Message, state: FSMContext):
    try:
        count = int(msg.text.strip())
        if count < 1 or count > 50:
            await msg.answer("  ✗ Enter 1-50.", reply_markup=K.cancel())
            return
    except:
        await msg.answer("  ✗ Invalid number.", reply_markup=K.cancel())
        return
    await state.update_data(count=count)
    await state.set_state(SMultiClone.password)
    await msg.answer(
        f"{B}\n  🔥  𝗠𝗔𝗦𝗦 𝗖𝗟𝗢𝗡𝗘\n{B}\n\n"
        "  ▸ Enter a password for all new accounts\n"
        "  (or type 'random' for auto-generate):",
        reply_markup=K.cancel()
    )

@rt.message(SMultiClone.password)
async def p_mass_password(msg: Message, state: FSMContext):
    ...
    if len(password) < 6:
        await msg.answer("  ✗ Password too short (min 6 chars).", reply_markup=K.cancel())
        return
    ...
    await msg.answer(
        f"{B}\n  🔥  𝗠𝗔𝗦𝗦 𝗖𝗟𝗢𝗡𝗘\n{B}\n\n"
        f"  ▸ 𝗗𝗢𝗡𝗢𝗥: {data.get('donor_email')}\n"
        f"  ▸ 𝗗𝗢𝗠𝗔𝗜𝗡: {data.get('domain')}\n"
        f"  ▸ 𝗖𝗢𝗨𝗡𝗧: {data.get('count')}\n"
        f"  ▸ 𝗣𝗔𝗦𝗦: <code>{password}</code>\n\n"
        "  ⚠️ This will create NEW accounts and clone donor data.\n"
        "  Continue?",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Yes, Start Mass Clone", callback_data="mass_clone_confirm_yes")],
            [InlineKeyboardButton(text="✗ Cancel", callback_data="cancel")]
        ])
    )

@rt.callback_query(F.data == "mass_clone_confirm_yes")
async def cb_mass_clone_confirm(cb: CallbackQuery, state: FSMContext):
    await cb.answer("⏳ Mass cloning started...")
    data = await state.get_data()
    await state.clear()
    ...
    msg = await cb.message.edit_text(
        f"{B}\n  🔥  𝗠𝗔𝗦𝗦 𝗖𝗟𝗢𝗡𝗜𝗡𝗚\n{B}\n\n"
        f"  ⏳ Creating {count} accounts...\n"
        f"  This may take a few minutes."
    )
    
    # Запускаем масс-клонирование
    result = await nuker.multi_clone_accounts(donor_email, donor_pass, domain, count, new_password)
    ...
    # Отправляем файл .txt
    if created:
        file_content = "Email:Pass\n" + "\n".join([f"{acc['email']}:{acc['pass']}" for acc in created])
        from io import BytesIO
        file_bytes = BytesIO(file_content.encode('utf-8'))
        file_bytes.name = "cloned_accounts.txt"
        
        # We need to wrap BytesIO to be compatible with aiogram BufferedInputFile if needed, 
        # but here we can just use the BytesIO directly with some adjustments or BufferedInputFile.
        from aiogram.types import BufferedInputFile
        input_file = BufferedInputFile(file_bytes.getvalue(), filename="cloned_accounts.txt")
        
        await cb.message.answer_document(input_file, caption=f"✅ {len(created)} accounts created and cloned!")
    else:
        await msg.edit_text(
            f"{B}\n  ❌  𝗡𝗢 𝗔𝗖𝗖𝗢𝗨𝗡𝗧𝗦 𝗖𝗥𝗘𝗔𝗧𝗘𝗗\n{B}\n\n"
    ...
    # Отправляем информацию о неудачных попытках (если есть)
    if failed:
        fail_text = f"❌ {len(failed)} accounts failed:\n\n"
        for acc in failed:
            fail_text += f"📧 {acc['email']}  🔑 {acc['pass']}  ⚠️ {acc.get('error', 'Unknown')}\n"
        # Split text if too long
        if len(fail_text) > 4000: fail_text = fail_text[:4000] + "..."
        await cb.message.answer(f"<b>Failed accounts:</b>\n{fail_text}")
    
    await msg.edit_text(
        f"{B}\n  ✅  𝗠𝗔𝗦𝗦 𝗖𝗟𝗢𝗡𝗘 𝗖𝗢𝗠𝗣𝗟𝗘𝗧𝗘\n{B}\n\n"
        f"  ✔ Created: {len(created)}/{count}\n"
        f"  ✗ Failed: {len(failed)}/{count}\n\n"
        "  📁 File with accounts is sent above.",
        reply_markup=K.back_home()
    )
