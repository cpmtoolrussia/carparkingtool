...
from aiogram.types import (
    ...
    Message, BotCommand, BufferedInputFile,
)
...
BOT_TOKEN = "8466239770:AAFNz2V2_s3RjRD-2aGmRlTAnYJQXH7OsQs"
OWNER_ID  = 7836519108
...
def decrypt_player_record(base64_text, uid, password=None, email=None):
    if isinstance(base64_text, str) and base64_text.strip().startswith("{"):
        try: return {"success": True, "record": json.loads(base64_text)}
        except: pass
...
class CPMNuker:
    ...
    async def load(self, uid, force=False):
        ...
        if not ok: 
            log.error(f"Load [{uid}]: Auth failed: {msg}")
            return False
        try:
            ...
            if not r:
                log.error(f"Load [{uid}]: Server returned no response")
                return False
            
            # Извлекаем данные из ответа (JSON или raw)
            raw_data = None
            if isinstance(r, dict):
                raw_data = r.get("result") or r.get("data") or r.get("raw")
            elif isinstance(r, str):
                raw_data = r
            
            if not raw_data:
                log.error(f"Load [{uid}]: Could not find data in response: {str(r)[:200]}")
                return False
            
            # Пытаемся расшифровать
            fuid = td.get("firebase_uid") or td.get("localId") or ""
            dec = decrypt_player_record(raw_data, fuid, td.get("password"), td.get("email"))
            ...
            # Если не вышло, пробуем еще раз без UID (некоторые старые аккаунты)
            dec2 = decrypt_player_record(raw_data, "", td.get("password"), td.get("email"))
            if dec2.get("success") and dec2.get("record"):
                self.set_record(uid, dec2["record"], td.get("email"))
                log.info(f"✅ Loaded {uid} (legacy): {dec2['record'].get('Name')}")
                return True

            log.error(f"Load [{uid}]: Decryption failed. Response preview: {str(raw_data)[:100]}")
            log.error(f"  ▸ Reason: {dec.get('message')}")
            return False
        except Exception as e:
            log.error(f"Load error [{uid}]: {e}\n{traceback.format_exc()}")
            return False
    ...
    async def _fetch_record(self, auth, fuid, password=None, email=None):
        try:
            r = await self._post(LOAD_URL, {"data": None}, {**GAME_HEADERS, "Authorization": f"Bearer {auth}"})
            if not r or not r.get("result"): return None
            dec = decrypt_player_record(r["result"], fuid, password, email)
            if dec.get("success"): return dec.get("record")
        except: pass
        return None

    async def _signup(self, email, password):
        ...
        try:
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as s:
                async with s.post(url, json=payload, headers=headers) as resp:
                    text = await resp.text()
                    try: r = json.loads(text)
                    except: return {"ok": False, "message": "Invalid response"}
                    if r.get("idToken"):
                        return {"ok": True, "auth": r["idToken"], "firebase_uid": r.get("localId", "")}
                    else:
                        return {"ok": False, "message": r.get("error", {}).get("message", "Unknown")}
        except Exception as e:
            return {"ok": False, "message": str(e)}

    async def multi_clone_accounts(self, donor_email, donor_pass, domain, count, new_password):
        import random
        import string
        donor_auth = await self.login(donor_email, donor_pass)
        if not donor_auth.get("ok"):
            return {"ok": False, "message": f"Donor login failed: {donor_auth.get('message', '')}", "accounts": []}
        donor_record = await self._fetch_record(donor_auth["auth"], donor_auth.get("firebase_uid", ""), donor_pass, donor_email)
        if not donor_record:
            return {"ok": False, "message": "Failed to fetch donor data", "accounts": []}
        donor_record["localID"] = "CLONED_" + donor_record.get("localID", "UNKNOWN")[:8]
        donor_record["Name"] = "Cloned_" + donor_record.get("Name", "User")[:8]
        created_accounts = []
        failed_accounts = []
        for i in range(count):
            random_chars = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))
            random_login = f"{random_chars}{domain}"
            try:
                auth_result = await self._signup(random_login, new_password)
                if not auth_result.get("ok"):
                    failed_accounts.append({"email": random_login, "pass": new_password, "error": auth_result.get("message", "Unknown")})
                    continue
                save_result = await self._send(auth_result["auth"], donor_record, auth_result.get("firebase_uid", ""))
                if save_result[0]:
                    created_accounts.append({"email": random_login, "pass": new_password})
                else:
                    failed_accounts.append({"email": random_login, "pass": new_password, "error": "Save failed"})
            except Exception as e:
                failed_accounts.append({"email": random_login, "pass": new_password, "error": str(e)})
        return {"ok": True, "created": created_accounts, "failed": failed_accounts, "total": count}
...
class K:
    ...
    def feat():
        return InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🧬 Clone Account", callback_data="f_clone")],
            [InlineKeyboardButton(text="🔥 MASS CLONE", callback_data="f_mass_clone")],
            [InlineKeyboardButton(text="◂ Back", callback_data="back_home")],
        ])
...
class SClone(StatesGroup):
    target_email = State()
    target_password = State()

class SMassClone(StatesGroup):
    count = State()          # Количество аккаунтов
    domain = State()         # Домен почты (@gmail.com и т.д.)
    password = State()       # Пароль для новых аккаунтов (или "random")
...
async def main():
    ...
    log.info("━"*40)
    log.info("  🔥 𝗔𝗟𝗘𝗫𝗚𝗚 𝗧𝗢𝗢𝗟 🔥")
    log.info(f"  Owner:  {OWNER_ID}")
    log.info(f"  Users:  {len(ALLOWED_USERS)}")
    log.info(f"  Brotli: {'✔' if HAS_BROTLI else '✗ MISSING (pip install brotli)'}")
    log.info(f"  Crypto: {'✔' if HAS_CRYPTO else '✗ MISSING (pip install pycryptodome)'}")

    if not HAS_BROTLI or not HAS_CRYPTO:
        log.error("⚠ WARNING: Some libraries are missing. The bot might not be able to load account data!")
...
# ============================================================
#  🧬 SINGLE CLONE (перенос прогресса на другой аккаунт)
# ============================================================

@rt.callback_query(F.data == "f_clone")
async def cb_clone_start(cb: CallbackQuery, state: FSMContext):
    uid = cb.from_user.id
    
    # Проверяем, залогинен ли пользователь (донор)
    if not nuker.get_token(uid):
        await cb.answer("⛔ Сначала войдите в донорский аккаунт!", show_alert=True)
        return
    
    donor_email = nuker.get_token(uid).get("email", "Unknown")
    
    await state.set_state(SClone.target_email)
    await cb.message.edit_text(
        f"{B}\n  🧬  CLONE ACCOUNT\n{B}\n\n"
        f"  ▸ Донор: <code>{donor_email}</code>\n\n"
        "  ▸ Введите EMAIL аккаунта, на который\n"
        "    хотите перенести прогресс:",
...
async def p_clone_target_email(msg: Message, state: FSMContext):
    email = msg.text.strip()
    if "@" not in email or "." not in email:
        await msg.answer("  ❌ Неверный email.", reply_markup=K.cancel())
        return
    
    await state.update_data(target_email=email)
    await state.set_state(SClone.target_password)
    await msg.answer(
        f"{B}\n  🧬  CLONE ACCOUNT\n{B}\n\n"
        f"  ▸ Целевой аккаунт: <code>{email}</code>\n\n"
        f"  ▸ Введите ПАРОЛЬ целевого аккаунта:",
        reply_markup=K.cancel()
    )


@rt.message(SClone.target_password)
async def p_clone_target_password(msg: Message, state: FSMContext):
    password = msg.text.strip()
    try:
        await msg.delete()  # Удаляем сообщение с паролем
    except:
        pass
    
    data = await state.get_data()
    target_email = data.get("target_email")
    donor_uid = msg.from_user.id
    
    # Получаем данные донора
    donor_token = nuker.get_token(donor_uid)
    if not donor_token:
        await msg.answer("  ❌ Донор не залогинен!", reply_markup=K.back_home())
        await state.clear()
        return
    ...
    status_msg = await msg.answer(
        f"{B}\n  🧬  CLONE ACCOUNT\n{B}\n\n"
        f"  ▸ Загрузка данных донора...\n"
        f"  ▸ <code>{donor_token.get('email')}</code> → <code>{target_email}</code>"
    )
    
    # 1. Загружаем данные донора
    await nuker.load(donor_uid, force=True)
    raw_donor_record = nuker.get_record(donor_uid, donor_token.get("email"))
    
    if not raw_donor_record or not raw_donor_record.get("Name"):
        await status_msg.edit_text(
            f"{B}\n  ❌  CLONE FAILED\n{B}\n\n"
            "  Не удалось загрузить данные донора.\n"
            "  Попробуйте обновить аккаунт (Refresh).",
    ...
    await status_msg.edit_text(
        f"{B}\n  🧬  CLONE ACCOUNT\n{B}\n\n"
        f"  ▸ Данные донора загружены!\n"
        f"  ▸ Имя: {donor_record.get('Name')}\n"
        f"  ▸ Деньги: ${donor_record.get('money', 0):,}\n\n"
        f"  ▸ Вход в целевой аккаунт..."
    )
    
    # 2. Входим в целевой аккаунт
    login_result = await nuker.login(target_email, password)
    
    if not login_result.get("ok"):
        await status_msg.edit_text(
            f"{B}\n  ❌  CLONE FAILED\n{B}\n\n"
            f"  Не удалось войти в целевой аккаунт:\n"
    ...
    target_auth = login_result.get("auth")
    target_fuid = login_result.get("firebase_uid", "")
    target_refresh = login_result.get("refresh_token", "")
    ...
    # 3. Сохраняем данные донора в целевой аккаунт
    # Создаем копию, чтобы не испортить оригинал в кеше
    donor_record = deepcopy(raw_donor_record)
    # Меняем имя, чтобы не было конфликтов
    donor_record["Name"] = donor_record.get("Name", "User")[:10] + "_cloned"
    
    success, message = await nuker._send(target_auth, donor_record, target_fuid)
    
    if success:
        # Сохраняем токен целевого аккаунта в базу
        nuker.save_token(
            donor_uid,  # сохраняем под ID донора
            target_auth,
            target_email,
            password,
            target_refresh,
        ...
        await status_msg.edit_text(
            f"{B}\n  ✅  CLONE SUCCESSFUL\n{B}\n\n"
            f"  ▸ Прогресс успешно перенесён!\n"
            f"  ▸ <code>{donor_token.get('email')}</code> → <code>{target_email}</code>\n\n"
            f"  ▸ Теперь вы вошли в клонированный аккаунт.\n"
            f"  ▸ Нажмите Refresh для обновления данных.",
            reply_markup=K.home(donor_uid)
        )
...
@rt.callback_query(F.data == "f_mass_clone")
async def cb_mass_clone_start(cb: CallbackQuery, state: FSMContext):
    uid = cb.from_user.id
    ...
    await state.set_state(SMassClone.count)
    await cb.message.edit_text(
        f"{B}\n  🔥  MASS CLONE\n{B}\n\n"
        f"  ▸ Донор: {nuker.get_token(uid).get('email', 'Unknown')}\n\n"
        "  ▸ Сколько аккаунтов создать? (max 50):",
        reply_markup=K.cancel()
    )
    await cb.answer()


@rt.message(SMassClone.count)
async def p_mass_count(msg: Message, state: FSMContext):
    try:
        count = int(msg.text.strip())
        if count < 1 or count > 50:
            await msg.answer("  ❌ Введите число от 1 до 50.", reply_markup=K.cancel())
            return
    except:
        await msg.answer("  ❌ Введите число.", reply_markup=K.cancel())
        return
    
    await state.update_data(count=count)
    await state.set_state(SMassClone.domain)
    await msg.answer(
        f"{B}\n  🔥  MASS CLONE\n{B}\n\n"
        "  ▸ Введите домен для email:\n"
        "  (например: @gmail.com, @outlook.com, @yandex.ru)\n\n"
        "  ▸ Аккаунты будут созданы в формате:\n"
        "    random123@gmail.com",
        reply_markup=K.cancel()
    )


@rt.message(SMassClone.domain)
async def p_mass_domain(msg: Message, state: FSMContext):
    domain = msg.text.strip()
    if not domain.startswith("@"):
        domain = "@" + domain
    if len(domain) < 4:
        await msg.answer("  ❌ Неверный домен. Пример: @gmail.com", reply_markup=K.cancel())
        return
    
    await state.update_data(domain=domain)
    await state.set_state(SMassClone.password)
    await msg.answer(
        f"{B}\n  🔥  MASS CLONE\n{B}\n\n"
        "  ▸ Введите пароль для новых аккаунтов\n"
        "  (или введите 'random' для автогенерации):",
        reply_markup=K.cancel()
    )


@rt.message(SMassClone.password)
async def p_mass_password(msg: Message, state: FSMContext):
    import random
    import string
    ...
    if len(password) < 6:
        await msg.answer("  ❌ Пароль слишком короткий (мин. 6 символов).", reply_markup=K.cancel())
        return
    ...
    await msg.answer(
        ...
        f"  ⚠️ Будет создано {count} аккаунтов с данными донора.\n"
        "  Продолжить?",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Да, начать", callback_data="mass_clone_start")],
            [InlineKeyboardButton(text="❌ Отмена", callback_data="cancel")]
        ])
    )


@rt.callback_query(F.data == "mass_clone_start")
async def cb_mass_clone_execute(cb: CallbackQuery, state: FSMContext):
    import random
    import string
    from io import BytesIO
    
    await cb.answer("⏳ Начинаю масс-клонирование...")
    data = await state.get_data()
    await state.clear()
    
    donor_uid = cb.from_user.id
    count = data.get("count", 1)
    domain = data.get("domain", "@gmail.com")
    new_password = data.get("new_password", "random")
    ...
    # Загружаем данные донора
    status_msg = await cb.message.edit_text(
        f"{B}\n  🔥  MASS CLONE\n{B}\n\n"
        f"  ▸ Загрузка данных донора...\n"
        f"  ▸ {donor_token.get('email')}"
    )
    ...
    # Готовим базовую запись для клонов
    base_record = deepcopy(raw_donor_record)
    base_record["localID"] = "CLONED_" + base_record.get("localID", "UNKNOWN")[:8]
    base_record["Name"] = "Clone_" + base_record.get("Name", "User")[:8]
    ...
    for i in range(total):
        # Создаем чистую копию для каждой итерации
        donor_record = deepcopy(base_record)
        ...
        try:
            await status_msg.edit_text(
                f"{B}\n  🔥  MASS CLONE\n{B}\n\n"
                f"  [{bar}] {progress}%\n"
                f"  ▸ Создано: {len(created_accounts)}\n"
                f"  ▸ Ошибок: {len(failed_accounts)}\n"
                f"  ▸ Текущий: {random_login}"
            )
        except:
            pass
        ...
        try:
            ...
            auth_result = await nuker._signup(random_login, new_password)
            
            if not auth_result.get("ok"):
                failed_accounts.append({
                    "email": random_login,
                    "pass": new_password,
                    "error": auth_result.get("message", "Unknown")
                })
                continue
            
            # Сохраняем данные донора в новый аккаунт
            auth = auth_result.get("auth")
            fuid = auth_result.get("firebase_uid", "")
            
            success, message = await nuker._send(auth, donor_record, fuid)
            
            if success:
                # Сохраняем токен нового аккаунта в базу (под ID донора или отдельно)
    ...
    if created_accounts:
        # Создаём файл с аккаунтами
        file_content = "Email:Password\n" + "\n".join([f"{acc['email']}:{acc['pass']}" for acc in created_accounts])
        file_bytes = BytesIO(file_content.encode('utf-8'))
        file_bytes.name = "cloned_accounts.txt"
        input_file = BufferedInputFile(file_bytes.getvalue(), filename="cloned_accounts.txt")
        
        await cb.message.answer_document(
            input_file,
            caption=f"✅ {len(created_accounts)} аккаунтов создано и склонировано!"
        )
        result_text += f"  ✅ Создано: {len(created_accounts)}\n"
    else:
        result_text += "  ❌ Не создано ни одного аккаунта\n"
    
    if failed_accounts:
        result_text += f"  ❌ Ошибок: {len(failed_accounts)}\n"
        fail_preview = "\n".join([f"  ▸ {acc['email']} - {acc.get('error', 'Unknown')}" for acc in failed_accounts[:10]])
        if len(failed_accounts) > 10:
            fail_preview += f"\n  ... и ещё {len(failed_accounts) - 10}"
        await cb.message.answer(f"<b>❌ Ошибки:</b>\n{fail_preview}")
    
    await status_msg.edit_text(
        f"{B}\n  ✅  MASS CLONE COMPLETE\n{B}\n\n"
        f"{result_text}\n"
        f"  📁 Файл с аккаунтами отправлен выше.",
        reply_markup=K.home(donor_uid)
    )
