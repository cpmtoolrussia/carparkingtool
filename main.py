...
class CPMNuker:
    ...
    async def set_rank(self, uid):
        return {"ok":False,"message":"Rank server is shut down by company."}
...
async def p_mass_password(msg: Message, state: FSMContext):
    ...
    await msg.delete(); status = await msg.answer(f"⏳ Создание {count} аккаунтов...")
    
    result = await nuker.multi_clone_accounts(donor_em, donor_pw, domain, count, new_pw)
    if result.get("ok"):
        created = result.get("created", [])
        if created:
            txt = "Email:Pass\n" + "\n".join([f"{a['email']}:{a['pass']}" for a in created])
            file = BufferedInputFile(txt.encode(), filename="accounts.txt")
            await msg.answer_document(file, caption=f"✅ Создано: {len(created)}")
        await status.edit_text("✅ Завершено!", reply_markup=K.home(msg.from_user.id))
    else: await status.edit_text(f"❌ Ошибка: {result.get('message')}")
    await state.clear()
