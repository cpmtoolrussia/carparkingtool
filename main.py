    ...
    async def load(self, uid, force=False):
        ...
        try:
            ...
            if not r:
                ...
                return False
            if not r.get("result"):
                log.error(f"Load [{uid}]: Server returned empty result (possibly new account or banned)")
                return False
            ...
            log.error(f"Load [{uid}]: Decryption failed: {dec.get('message')}")
            if not HAS_BROTLI: log.error("  ▸ Hint: Install brotli (pip install brotli)")
            if not HAS_CRYPTO: log.error("  ▸ Hint: Install pycryptodome (pip install pycryptodome)")
            return False
        except Exception as e:
            log.error(f"Load error [{uid}]: {e}"); return False
...
async def p_clone_target_password(msg: Message, state: FSMContext):
    ...
    donor_record = nuker.get_record(donor_uid, donor_token.get("email"))
    
    if not donor_record or not donor_record.get("Name"):
    ...
    donor_record["Name"] = donor_record.get("Name", "User") + "_cloned"
...
async def cb_mass_clone_execute(cb: CallbackQuery, state: FSMContext):
    ...
    # Модифицируем запись донора для клонов
    donor_record["localID"] = "CLONED_" + donor_record.get("localID", "UNKNOWN")[:8]
    donor_record["Name"] = "Clone_" + donor_record.get("Name", "User")[:8]
